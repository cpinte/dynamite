...
class Cube:
    ...
    def _read(self):
        try:
            ...
            self.pixelscale = hdu[0].header['CDELT2'] * 3600. # arcsec : we assume square pixels
            ...
            if (self.velocity_type == "VELO-LSR"): # gildas format m/s -> km/s
                ...
                self.nu = self.CRVAL3 + self.CDELT3 * (np.arange(1,self.nv+1) - self.CRPIX3) # Hz : tested ok
                self.velocity = -(self.nu - self.restfreq)/self.restfreq * sc.c / 1000. # km/s
            else:
    ...
    def plot(self,iv=None,v=None,colorbar=True,plot_beam=True,color_scale=None,fmin=None,fmax=None,limit=None,limits=None,moment=None,Tb=False,cmap=None,v0=None,dv=None,ax=None,no_ylabel=False,no_xlabel=False,no_vlabel=False,title=None,dx=0.0,dy=0.0,alpha=1.0,resample=0, bmaj=None, bmin=None, bpa=None, taper=None,colorbar_label=True,M0_threshold=None,vlabel_position="bottom"):
        ...
        if (taper is not None):
            if (taper < self.bmaj):
                ...
                delta_bmaj = np.sqrt(taper**2 - self.bmaj**2) # sigma will be 1 pixel
                bmaj = taper
            if (taper < self.bmin):
        ...
        image = ax.imshow(im, norm=norm, extent=self.extent - np.asarray([dx,dx,dy,dy]),
                          origin='lower',cmap=cmap,alpha=alpha, interpolation="bicubic")
        ...
        if (not no_vlabel):
            if (moment is None) and not is_cont:
                if (v0 is None):
                    ax.text(0.5,y_vlabel,f"v={self.velocity[iv]:<4.2f}$\,$km/s",horizontalalignment='center',color="white",transform=ax.transAxes)
                else:
                    ax.text(0.5,y_vlabel,f"$\Delta$v={self.velocity[iv] -v0:<4.2f}$\,$km/s",horizontalalignment='center',color="white",transform=ax.transAxes)
    ...
    def get_moment_map(self, moment=0, v0=0, M0_threshold=None):
        ...
        if moment in [1,2]:
            M1 = np.sum(cube[:, :, :] * v[:, np.newaxis, np.newaxis], axis=0) * np.abs(dv) / M0
        ...
        if moment == 2:
            M2 = np.sqrt(np.sum(cube[:, :, :] * (v[:, np.newaxis, np.newaxis] - M1[np.newaxis, :, :]) ** 2,axis=0,) * np.abs(dv) / M0)
            return M2
    ...
    def _beam_area(self):
        ...
        return np.pi * self.bmaj * self.bmin / (4.*np.log(2.))
    ...
    def _Jybeam_to_Tb(self,im):
        ...
        exp_m1 = 1e26 * self._beam_area_str() * 2. * sc.h * nu**3 / (sc.c**2 * abs(im2))
